...
from klassify.src.doc_finder import DocFinder
import json
import subprocess
import os
import pytest
...
DOC_FINDER = DocFinder(db_name=database_name)
...
def test_associate_document_with_subtopics():
    made_document = DOC_FINDER.make_document(DOCUMENT_DATA)
    DOC_FINDER.associate_document_with_subtopics(made_document, SUBTOPICS)
    assert made_document.subtopics[0] in SUBTOPICS
    assert made_document.subtopics[1] in SUBTOPICS

def test_find_subtopics():

    session = DOC_FINDER.session
    session.add_all(SUBTOPICS)
    session.commit()

    found_topics = DOC_FINDER.find_subtopics(DOCUMENT_DATA)
    ...
    DOC_FINDER.DBH.destroy_db_if_present()
