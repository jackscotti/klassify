...
DOC_IMPORTER = DocumentImporter(db_name=database_name)

def test_total_documents():
    assert DOC_IMPORTER.total_documents(api_response_fixture) == 9623

def test_pages():
    assert DOC_IMPORTER.pages(9623) == 10

def test_urls():
    assert DOC_IMPORTER.urls(10) == [
...
def test_make_document():
    made_document = DOC_IMPORTER.make_document(DOCUMENT_DATA)
...
def test_associate_document_with_subtopics():
    made_document = DOC_IMPORTER.make_document(DOCUMENT_DATA)
    DOC_IMPORTER.associate_document_with_subtopics(made_document, SUBTOPICS)
    assert made_document.subtopics[0] in SUBTOPICS
    assert made_document.subtopics[1] in SUBTOPICS

def test_find_subtopics():

    session = DOC_IMPORTER.session
    session.add_all(SUBTOPICS)
    session.commit()

    found_topics = DOC_IMPORTER.find_subtopics(DOCUMENT_DATA)
    ...
    DOC_IMPORTER.DBH.destroy_db_if_present()
